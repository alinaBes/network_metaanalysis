...
data_raw |> 
  ...
  ungroup() |> 
  filter(n_endpoints > 1) |> select(study, n_endpoints)
...
study_endpoints <- data_raw |> 
  group_by(study) |> 
  summarise(
    across(starts_with("n_events"), ~ first(na.omit(.))),
    .groups = "drop"
  ) |> 
  mutate(
    n_events_selected = coalesce(
      `n_events_28-30`,
      n_events_60,
      n_events_90,
      n_events_ICU,
    ...
    endpoint_used_28 = case_when(
      !is.na(`n_events_28-30`) ~ "28-30",
      !is.na(n_events_60)    ~ "60",
      !is.na(n_events_90)    ~ "90",
      !is.na(n_events_ICU)   ~ "ICU",
      !is.na(n_events_hospt)   ~ "hospt",
      !is.na(n_events_last) ~ "last",
      TRUE ~ NA_character_
    ),
    endpoint_used_max = case_when(
      !is.na(n_events_hospt)   ~ "hospt",
      !is.na(n_events_ICU)   ~ "ICU",
      !is.na(n_events_last) ~ "last",
      !is.na(n_events_90)    ~ "90",
  ...
  ) |> select(study, endpoint_used_28, endpoint_used_max)
...
date_28 <- data_with_endpoints |> 
  rowwise() |> 
  mutate(
    events  = get(paste0("n_events_", endpoint_used_28)),
    at_risk = get(paste0("n_at_risk_", endpoint_used_28))
  ) |> 
  ungroup() |> select(study, group_code, n_events = events, n_patients = at_risk)


max_date <- data_with_endpoints |> 
  rowwise() |> 
  mutate(
    events  = get(paste0("n_events_", endpoint_used_max)),
    at_risk = get(paste0("n_at_risk_", endpoint_used_max))
  ) |> 
  ungroup() |>  select(study, group_code, n_events = events, n_patients = at_risk)
